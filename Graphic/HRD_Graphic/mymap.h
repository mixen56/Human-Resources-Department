...
public:
    ...
    bool operator !=(MyIterator<Type1, Type2> rigth);
    bool operator ==(MyIterator<Type1, Type2> rigth);
...
public:
	...
	int size() { return SZ; }
    bool empty() const { if (SZ == 0) return 1; else return 0; }
};

//using namespace std;
//реализация
template <typename Type1, typename Type2>
bool MyIterator<Type1, Type2>::operator != (MyIterator<Type1, Type2> rigth)
{
	if (now == rigth.now)
		return 0;
	else
